...
```{r step4, echo=TRUE, eval=TRUE}
ggplot(df, aes(SATV, SATQ)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE) +
  stat_cor(method = "pearson", label.x = 200, size = 6) +
  labs(
    x = "SAT Verbal",
    y = "SAT Quantitative",
    title = "SAT Verbal vs SAT Quantitative"
...
![](_assets/img/scatter_correlation_sat_act.png)

---

## Hidden solution

```{r solution, echo=FALSE, eval=TRUE}
ggplot(df, aes(SATV, SATQ)) +
  geom_point() +
